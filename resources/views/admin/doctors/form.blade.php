...
{!! Form::model($doctor, [
	'method' => !$doctor->exists ?: 'put',
    'route' => $doctor->exists ? ['admin.doctors.update', $doctor] : 'admin.doctors.store',
    'enctype' => 'multipart/form-data'
    ]) !!}
    <div class="row">
        @if($doctor->picture)
            <div class="col-12">
                <div class="image-preview"><img src="{{ old('inputPicture') ?: $doctor->picture }}" /></div>
            </div>
        @endif
        <div class="col-12">
            <div class="form-group">
                <label>Profile Picture</label>
                <input type="hidden" name="inputPicture" id="inputPicture" value="{{ old('inputPicture') }}">
        ...
        <div class="col-12 col-md-6 col-lg-3">
            <div class="form-group">
                ...
                id="idNumber" value="{{ old('id_number', $doctor->id_number) }}">
                @error('id_number')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="row">
                <div class="col pr-0">
                    <div class="form-group">
                        ...
                        <select name="specializations[]" class="form-control @error('specializations') is-invalid @enderror" id="specializationDropdown" data-action="{{ route('admin.specializations.index') }}" data-value="{{ old('specializations') ? implode(',', old('specializations')) : $doctor->specializations()->pluck('specializations.id')->implode(',') }}">
                            {{-- {{ $new ? implode(',', old('specializations') ?: array()) : $doctor->specializations()->pluck('specializations.id')->implode(',') }} --}}
                        </select>
                ...
                <div class="col-auto" style="margin-top: 30px">
                    <a href="javascript:void(0);" data-toggle="modal" data-target="#specializationModal" class="btn btn-primary">Add Specialization</a>
                </div>
            </div>
        </div>
        <div class="col-12 col-md-6">
            <div class="row">
                <div class="col pr-0">
                    <div class="form-group">
                        <label for="accreditationDropdown">HMO Accreditations</label>
                        <select name="hmos[]" class="form-control @error('hmos') is-invalid @enderror" id="accreditationDropdown" data-action="{{ route('admin.hmo-accreditations.index') }}" data-value="{{ old('hmos') ? implode(',', old('hmos')) : $doctor->hmos()->pluck('hmo_accreditations.id')->implode(',') }}">
                            {{-- {{ $new ? implode(',', old('hmos') ?: array()) : $doctor->hmos()->pluck('hmo_accreditations.id')->implode(',') }} --}}
                        </select>
                        @error('hmos')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="col-auto" style="margin-top: 30px">
                    <a href="javascript:void(0);" data-toggle="modal" data-target="#accreditationModal" class="btn btn-primary">Add HMO Accreditation</a>
                </div>
            </div>
        </div>
        <div class="col-12 col-md-6">
        ...
        <div class="col">
            <div class="col-auto my-1">
                <div class="custom-control custom-checkbox mr-sm-2">
                    <input type="checkbox" name="status" class="custom-control-input" id="active" value="1" {{ (old('status', optional($doctor->user)->status) == 1) || ($new == true) ? 'checked' : '' }}>
...
<div class="modal fade" id="specializationModal" tabindex="-1" role="dialog" aria-labelledby="specializationModalLabel" aria-hidden="true">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                @include('admin.specializations.form', [
                    ...
                    'parents' => App\Specialization::getParents(),
                ])
            </div>
        </div>
    </div>
...
<div class="modal fade" id="accreditationModal" tabindex="-1" role="dialog" aria-labelledby="accreditationModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="accreditationModalLabel">Add HMO Accreditations</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
                </button>
            </div>
...
<script>
    ...
    function cropImage() {
        ...
        $("#saveImage").click(function(){
            ...
            const parentForm = $('body').find('form').eq(1);
            parentForm.find('button[type="submit"].btn-primary').attr('disabled', true);
            if(canvas){
                ...
                setTimeout(function(){
                    ...
                    parentForm.find('button[type="submit"].btn-primary').attr('disabled', false);
                }, 2000);
