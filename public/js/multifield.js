      ...
      if(this.config.locale !== 'default'){
		...
		cloneSection : function() {
			...
			$('input[type!="radio"],textarea,select', newChild).each(function () {
				...
				$(this).val('');
                // Update by kaito - increment custom data attributes
                var data_validation_depends_on = $(this).attr('data-validation-depends-on');
                if(data_validation_depends_on){
                    $(this).attr('data-validation-depends-on', data_validation_depends_on.replace(/([0-9]+)/g,1*(data_validation_depends_on.match(/([0-9]+)/g))+1));
                }
                $(this).css('border', '');
			});
