...
class AppointmentController extends Controller
{
    ...
    public function details(Consultation $consultation, Request $request)
    {
        ...
        // if (!session()->has('edit_c')) {
            session(['edit_c' => $consultation->id]);
        // }
    ...
    {
        abort_if(session('edit_c') != $consultation->id, 401);
    ...
    {   
        ...
        $doctorPaymentInstruction = $doctor->payment_instructions;
        if (count($recepients) > 0) {
            Mail::to($recepients, $doctor->full_name)->queue(new \App\Mail\AppointmentCreated($c, $doctorPaymentInstruction));
        }
        Mail::to($c->email)->queue(new \App\Mail\AppointmentCreatedReceipt($c, $doctorPaymentInstruction));

        // sms for patient
        if ($c->mobile) {
            SendSmsJob::dispatch('patient_doctor_booked', str_replace('-', '', $c->mobile), [
        ...
        //         SendSmsJob::dispatch('doctor_doctor_booked', str_replace('-', '', $mobile), [
